...
#     pose_record = loadPoses() # empty dict initially
    ...
    for i in range(2919, len(impaths)-1):    
        ...
        if load_sift == False:
            ...
            pts1, pts2 = loadSIFTmatches(i, siftFilepath = BasePath + 'data/sift/')
        
        data = (pts1,pts2)
        F,inlier_mask = FundamentalMatrix(data ,s = 8, thresh = 0.0001,n_iterations = 75)
        pts1_ = pts1[inlier_mask==1]
        pts2_ = pts2[inlier_mask==1]    
        saveSIFTmatches(i, pts1_, pts2_, siftFilepath = BasePath + 'data/sift_inliers/')
        
        E = EssentialMatrix(K, F)
        
        if str(i) not in E_record:
            E_record[str(i)] = E
            saveDict(E_record, 'Essential_1', Filepath = BasePath + 'data/record/')
        else:
